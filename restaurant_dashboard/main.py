...
def heatmap(df):
    ny_df = pd.to_csv('ny_df.csv', sep=",", index_col=0)
    sf_df = pd.to_csv('sf_df.csv', sep=",", index_col=0)
...
def main():
    if __name__ == '__main__':
        ...
        #ny_df = pd.read_csv('data/ny_df.csv', sep=',', index_col=0)
        #sf_df = pd.read_csv('data/sf_df.csv', sep=',', index_col=0)

        #st.markdown('#### Heatmap for New York')
        #st.write("##### Intensity based on number of orders per restaurant")
        #heatmap(ny_df)

        #st.markdown('#### Heatmap for San Francisco')
        #st.write("##### Intensity based on number of orders per restaurant")
        #heatmap(sf_df)
